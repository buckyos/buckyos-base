...
// Provider工厂 - 返回闭包
pub type ProviderFnFactory =
    Arc<dyn Fn(&serde_json::Value) -> Result<ProviderFn, String> + Send + Sync>;
...
impl ProviderFnRegistry {
    ...
    ) -> Result<ProviderFn, String> {
        self.factories
            .get(provider_type)
            .ok_or_else(|| format!("未知的Provider类型: {}", provider_type))?(config)
    }
}
...
// 全局函数用于创建Provider
pub fn create_provider_by_config(
    provider_type: &str,
    config: &serde_json::Value,
) -> Result<ProviderFn, String> {
