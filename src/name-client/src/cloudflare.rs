...
impl CloudflareProvider {
    ...
    async fn get_zone_id(&self, domain: &str) -> NSResult<String> {
        ...
        let zones: CloudflareResponse<Vec<Zone>> = response
            .json()
            .await
    ...
    async fn make_request(
        ...
        req.send()
            .await
            .map_err(|e| NSError::Failed(format!("Request failed: {}", e)))
    }
}
...
impl NsUpdateProvider for CloudflareProvider {
    async fn update(&self, record_type: RecordType, record: NameInfo) -> NSResult<NameInfo> {
        ...
        let body = serde_json::to_vec(&dns_record)
            .map_err(|e| NSError::Failed(format!("Failed to serialize request: {}", e)))?;
        let response = self
            .make_request(&record.name, Method::POST, "", Some(body))
            .await?;

        let cf_response: CloudflareResponse<DnsRecord> = response
            .json()
            .await
    ...
    async fn delete(&self, name: &str, record_type: RecordType) -> NSResult<Option<NameInfo>> {
        ...
        let response = self
            .make_request(name, Method::GET, &list_path, None)
            .await?;

        let list_response: CloudflareResponse<Vec<DnsRecord>> = response
            .json()
            .await
            .map_err(|e| NSError::Failed(format!("Failed to parse response: {}", e)))?;
        ...
        if let Some(records) = list_response.result {
            for record in records {
                ...
                let response = self
                    .make_request(name, Method::DELETE, &delete_path, None)
                    .await?;

                let delete_response: CloudflareResponse<DnsRecord> = response
                    .json()
                    .await
                    .map_err(|e| NSError::Failed(format!("Failed to parse response: {}", e)))?;
...
impl NsProvider for CloudflareProvider {
    ...
    async fn query(
        ...
        let response = self
            .make_request(name, Method::GET, &path, None)
            .await
            .map_err(|e| NSError::Failed(format!("Request failed: {}", e)))?;
        let cf_response: CloudflareResponse<Vec<DnsRecord>> = response
            .json()
            .await
            .map_err(|e| NSError::Failed(format!("Failed to parse response: {}", e)))?;
    ...
    async fn query_did(
        ...
        Err(NSError::Failed(
            "DID query not supported by Cloudflare provider".to_string(),
        ))
    }
}
